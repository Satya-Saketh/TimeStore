        ...
        <div className={contactPageStyles.pageContainer}>
            <div className={contactPageStyles.innerContainer}>
                <div className={contactPageStyles.pageHeader}>
                    <h1 className={contactPageStyles.pageTitle} style={{
                ...
                <div className={contactPageStyles.mainGrid}>
                        ...
                        <div className={contactPageStyles.formCard}>
                            <form onSubmit={handleSubmit} className={contactPageStyles.form}>
                                <div className={contactPageStyles.inputGrid}>
                                    ...
                                    <InputWithIcon icon={<User className="w-5 h-5 text-black" />} label="Your Name" name="name" value={form.name}
                                        onChange={handleChange} placeholder="FullName" error={errors.name} required />
                                    <InputWithIcon icon={<Mail className="w-5 h-5 text-black" />} label="Email" name="email" value={form.email}
                                        onChange={handleChange} placeholder="Email" error={errors.email} required />
                                </div>
                                <div className={contactPageStyles.inputGrid}>
                                    <InputWithIcon icon={<Phone className="w-5 h-5 text-black" />} label="Phone" name="phone" value={form.phone}
                                        onChange={handleChange} placeholder="Phone Number" error={errors.phone} required />
                                    <SelectWithIcon icon={<Clock className="w-5 h-5 text-black" />} label="Preferred Contact" name="contactMethod" value={form.contactMethod}
                                        onChange={handleChange} options={["WhatsApp", "Phone Call", "Email"]} error={errors.contactMethod} required />
                                </div>
                                <div>
                                    <SelectWithIcon icon={<ShoppingCart className="w-5 h-5 text-black" />} label="Product of Interest" name="product" value={form.product}
                                        onChange={handleChange} options={products} error={errors.product} required />
                                </div>
                                <div className={contactPageStyles.inputGrid}>
                                    <InputWithIcon icon={<IndianRupee className="w-5 h-5 text-green-600" />} label="Estimated Budget" name="budget" value={form.budget}
                                        onChange={handleChange} placeholder="e.g. ₹1,50,000 - ₹3,00,000" error={errors.budget} required />
                                    <div>
                    ...
                    <div className={contactPageStyles.rightColumn}>
                        <div className={contactPageStyles.rightColumnGrid}>
                            <CreativeCard title="Showroom Visits" subtitle="Private viewings by appointment"
                                icon={<MapPin className='w-6 h-6 text-black' />} ctaText="Book Visit"
                                ctaOnClick={() => {
